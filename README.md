...
短期メモのうち**ゴミ箱へ移動するまで2日以内**のものを、**朝9時（日本時間）**に
**まとめて1通**お知らせします。

通知はこの形です。

```
2日以内に消去予定のメモがあります【3件】
本日中 1件：買い物メモ
2日以内 2件：会議の要点 / 無題のメモ
```
...
- **条件を満たす限り毎日届きます。** 「ちょうど2日前」の一点だけを狙うと、保存日数を
  短くしたメモ（1日など）がその瞬間を飛び越えてしまい一度も通知されないためです。
- 残り日数は**暦日**で数えます（「本日中」＝今日が期限の日）。
- 各行には、そのメモのタイトルを最大3件まで並べ、超えた分は「ほか◯件」にまとめます。
  タイトル未入力のメモは一覧と同じく本文1行目、それも無ければ「無題のメモ」になります。
- メモの本文は通知に含めません（ロック画面に出るため）。
- タップすると短期メモ一覧が開きます（対象が1件だけのときはそのメモが直接開きます）。
- 配信は Vercel Cron が毎日 UTC 0:00（＝日本時間 9:00）に
  `/api/cron/expiry-notify` を呼んで行います（[`vercel.json`](vercel.json)）。
- `push_notifications_sent` は「最後にいつ通知したか」の記録です。配信の判定には
  使っておらず、バッチが動いたかを確認するために残しています。
